  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      const playNext = async () => {
        if (allAnnouncements.length === 0 || !isActive || !isMounted) return;
        ...
        while (!spoken && attempts < allAnnouncements.length) {
          if (currentPlayId !== playId || !isActive || !isMounted) return;
        ...
        if (isActive && isMounted && currentPlayId === playId) {
      ...
      const handleScroll = () => {
        ...
        if (Math.abs(currentScrollY - lastScrollY) > 500) {
          ...
          timeoutId = setTimeout(playNext, 1500); // Start next vendor after brief pause
        }
      };
