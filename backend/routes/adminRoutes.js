...
router.get("/pending-users", async (req, res) => {
  try {
    ...
    const __dir = join(fileURLToPath(import.meta.url), "../../public/uploads");

    // Get list of all upload files to support legacy registrations
    let uploadFiles = [];
    try { uploadFiles = readdirSync(__dir); } catch { uploadFiles = []; }
    ...
    for (const user of pendingUsers) {
      if (user.role === "farmer") {
        ...
        // Filesystem fallback: find files matching fieldname pattern near user creation time
